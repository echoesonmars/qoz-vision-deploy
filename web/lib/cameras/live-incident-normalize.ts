...
const KNOWN = new Set<string>(INCIDENT_CATEGORIES);

const ALIASES: Record<string, IncidentCategory> = {
  phone: "phone_usage",
  phones: "phone_usage",
...
export function normalizeLiveIncidentType(raw: string): IncidentCategory {
  const s = slug(raw);
  if (!s) return "intruder";
  if (KNOWN.has(s)) return s as IncidentCategory;
